import { carouselProps, carouselPartsProps } from "@/components/demos/carousel/carousel-props";
...
- Off-screen slides are `aria-hidden` and `inert`, so focus can't leak into
  them.
- Indicators form a real tablist; arrow keys step slides, following the
  orientation.
