...
interface ComponentPreviewProps {
  /** Registry key of the demo (see src/components/demos/index.ts). */
...
export function ComponentPreview({ name, align }: ComponentPreviewProps) {
  ...
  if (!demo) {
    throw new Error(`[docs] Unknown demo "${String(name)}" — add it to src/components/demos/index.ts`);
  }

  const source = fs.readFileSync(
    path.join(process.cwd(), "src", "components", "demos", demo.file),
    "utf8",
  );
